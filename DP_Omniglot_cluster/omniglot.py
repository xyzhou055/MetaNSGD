...
class AbstractMetaOmniglot(object):
    ...
    def delete_empty_class(self):
        for i in range(len(self.characters_list)):
            ch, path = self.characters_list[i]
            if len(path)==0:
                print(ch)
    ...
    def get_task_split(self, character_indices, img_indices, train_K, test_K):
        """
        return train_task and test_task given the choice of character and img_indices
        character_indices: arg.class *1 array
        img_indices: arg.class * (train_K + test_K)
        """
        train_samples = []
        test_samples = []
        for base_idx, idx in enumerate(character_indices):
            character, paths = self.characters_list[idx]
            for i, path_idx in enumerate(img_indices[base_idx]):
                new_path={}
                new_path.update(paths[path_idx])
                new_path['base_idx'] = base_idx
                if i < train_K:
                    train_samples.append(new_path)
                else:
